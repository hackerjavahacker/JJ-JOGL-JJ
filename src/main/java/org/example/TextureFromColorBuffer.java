...
public class TextureFromColorBuffer extends JPanel implements GLEventListener {
    ...
    public static void main(String[] args) {
        JFrame window = new JFrame("Hakim Khan");
    ...
    private JSlider side = new JSlider(1, 50, 25);
    private JPanel radiusPanel = new JPanel();
    private JSlider radius = new JSlider(1, 50, 25);
    private JPanel slicesPanel = new JPanel();
    private JSlider slices = new JSlider(1, 50, 25);
    private JPanel stacksPanel = new JPanel();
    private JSlider stacks = new JSlider(1, 50, 25);
    private JPanel heightPanel = new JPanel();
    private JSlider height = new JSlider(1, 50, 25);
    private JPanel ringsPanel = new JPanel();
    private JSlider rings = new JSlider(0, 100, 50);
    private JPanel outerRadiusPanel = new JPanel();
    private JSlider outerRadius = new JSlider(2, 50, 35);
    private JPanel innerRadiusPanel = new JPanel();
    private JSlider innerRadius = new JSlider(1, 49 , 20);
    GLUT glut = new GLUT();
    ...
    public TextureFromColorBuffer() {
        ...
        sidePanel.add(side);
        sidePanel.setMaximumSize(new Dimension(500, 25));
        right.add(sidePanel);
        side.addChangeListener(new ChangeListener() {
            @Override
        ...
        radiusPanel.setMaximumSize(new Dimension(500,25));
        right.add(radiusPanel);
        ...
        right.add(Box.createVerticalStrut(25));
        slicesPanel.setLayout(new FlowLayout(FlowLayout.CENTER));
        slicesPanel.add(new JLabel("slices"));
        slicesPanel.add(slices);
        slicesPanel.setMaximumSize(new Dimension(500,25));
        right.add(slicesPanel);
        slices.addChangeListener(new ChangeListener() {
            @Override
            public void stateChanged(ChangeEvent e) {
                repaint();
            }
        ...
        right.add(Box.createVerticalStrut(25));
        stacksPanel.setLayout(new FlowLayout(FlowLayout.CENTER));
        stacksPanel.add(new JLabel("stacks"));
        stacksPanel.add(stacks);
        stacksPanel.setMaximumSize(new Dimension(500,25));
        right.add(stacksPanel);
        stacks.addChangeListener(new ChangeListener() {
            @Override
            public void stateChanged(ChangeEvent e) {
                repaint();
            }
        ...
        heightPanel.setMaximumSize(new Dimension(500, 25));
        ...
        right.add(Box.createVerticalStrut(25));
        ringsPanel.setLayout(new FlowLayout(FlowLayout.CENTER));
        ringsPanel.add(new JLabel("rings"));
        ringsPanel.add(rings);
        ringsPanel.setMaximumSize(new Dimension(500, 25));
        right.add(ringsPanel);
        rings.addChangeListener(new ChangeListener() {
            @Override
            public void stateChanged(ChangeEvent e) {
                repaint();
            }
        ...
        right.add(Box.createVerticalStrut(25));
        outerRadiusPanel.setLayout(new FlowLayout(FlowLayout.CENTER));
        outerRadiusPanel.add(new JLabel("outer"));
        outerRadiusPanel.add(outerRadius);
        outerRadiusPanel.setMaximumSize(new Dimension(500, 25));
        right.add(outerRadiusPanel);
        outerRadius.addChangeListener(new ChangeListener() {
            @Override
            public void stateChanged(ChangeEvent e) {
                if (outerRadius.getValue() <= innerRadius.getValue()) {
                    innerRadius.setValue(outerRadius.getValue() - 1);
                }
                repaint();
            }
        });
        ...
        right.add(Box.createVerticalStrut(25));
        innerRadiusPanel.setLayout(new FlowLayout(FlowLayout.CENTER));
        innerRadiusPanel.add(new JLabel("inner"));
        innerRadiusPanel.add(innerRadius);
        innerRadiusPanel.setMaximumSize(new Dimension(500,25));
        right.add(innerRadiusPanel);
        innerRadius.addChangeListener(new ChangeListener() {
            @Override
            public void stateChanged(ChangeEvent e) {
                if (innerRadius.getValue() >= outerRadius.getValue()) {
                    outerRadius.setValue(innerRadius.getValue() + 1);
                }
                repaint();
            }
        });
    ...
    public void display(GLAutoDrawable drawable) {
        ...
        double sideValue = (double) side.getValue() / 25;
        double radiusValue = (double) radius.getValue() / 50;
        int slicesValue = slices.getValue();
        int stacksValue = stacks.getValue();
        double heightValue = (double) height.getValue() / 25;
        int ringsValue = rings.getValue();
        double outerRadiusValue = (double) outerRadius.getValue() / 50;
        double innerRadiusValue = (double) innerRadius.getValue() / 50;

        switch (selectedObject) {
            case 0:
                TexturedShapes.uvSphere(gl, radiusValue, slicesValue > 3 ? slicesValue : 3, stacksValue > 2 ? stacksValue : 2);
                setVisibility(false, true, true, true, false, false, false, false);
                break;
            case 1:
                TexturedShapes.uvCylinder(gl, radiusValue, heightValue, slicesValue > 3 ? slicesValue : 3, stacksValue > 2 ? stacksValue : 2, ringsValue);
                setVisibility(false, true, true, true, true, true, false, false);
                break;
            case 2:
                TexturedShapes.cube(gl, sideValue);
                setVisibility(true, false, false, false, false, false, false, false);
                break;
            case 3:
                TexturedShapes.uvCone(gl, radiusValue, heightValue, slicesValue > 3 ? slicesValue : 3, stacksValue > 2 ? stacksValue : 2, ringsValue / 10);
                setVisibility(false, true, true, true, true, true, false, false);
                break;
            case 4:
                TexturedShapes.uvTorus(gl, outerRadiusValue, innerRadiusValue, slicesValue > 3 ? slicesValue : 3, ringsValue > 3 ? ringsValue : 3);
                setVisibility(false, false, true, true, false, true, true, true);
                break;
            case 5:
                glut.glutSolidTeapot(radiusValue);
                setVisibility(false, true, false, false, false, false, false, false);
        }
    ...
    public void setVisibility(boolean sideVisibility, boolean radiusVisibility, boolean slicesVisibility,
                              boolean stacksVisibility, boolean heightVisibility, boolean ringsVisibility,
                              boolean outerRadiusVisibility, boolean innerRadiusVisibility) {
        sidePanel.setVisible(sideVisibility);
        radiusPanel.setVisible(radiusVisibility);
        slicesPanel.setVisible(slicesVisibility);
        stacksPanel.setVisible(stacksVisibility);
        heightPanel.setVisible(heightVisibility);
        ringsPanel.setVisible(ringsVisibility);
        outerRadiusPanel.setVisible(outerRadiusVisibility);
        innerRadiusPanel.setVisible(innerRadiusVisibility);
    }
}
